...
export default class {
    ...
    static getFetchCount () {
        return fetchCount
    }

    constructor (params) {
        let {job, url, userAgent, done, dateFormat, channelName, cache, whitelist, maxConcurrency, interval, pushActions} = params
        this.done = done
        ...
        this.crawler.maxConcurrency = maxConcurrency
        this.crawler.maxDepth = 2
        this.crawler.userAgent = userAgent
        this.crawler.interval = interval
        ...
        if (news) {
            ...
            News.findOne({contentId: news.contentId}).then((_news) => {
                if (!_news) { // 创建新闻
                    ...
                    logger.debug(`Push news actions.`)
                    // Push.notificate({title: news.title, url: news.url})
                    this.pushActions.forEach((pushAction) => {
                        pushAction(news)
                    })
                    ...
                    _news.channelItems.forEach((item) => { // TODO lodash简化写法
                        if (item.name == this.channelName) {
                            logger.debug(`News is same, this.channel: ${this.channelName}, item.channel: ${item.name}`)
                            isSameChannel = true
                            return
                        }
                    })
